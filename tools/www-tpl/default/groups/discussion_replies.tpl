{% if hasMessage == false %}
	<div class="postlist-header clearfix">
				...
				<span class="topic-closed"><img src="{{ site.staticContentPath }}/web-gallery/images/groups/status_closed.gif" title="{{ locale.groups_discussion_replies_closed_thread }}"> {{ locale.groups_discussion_replies_closed_thread }}</span>
			{% endif %}
				{% if canReplyForum %}
                    <a href="#" id="create-post-message" class="create-post-link verify-email">{{ locale.groups_discussion_replies_post_reply }}</a>
										{% endif %}
					{% if (session.loggedIn) and ((playerDetails.id == discussionTopic.getCreatorId()) or hasTopicAdmin) %}
                <a href="#" id="edit-topic-settings" class="edit-topic-settings-link">{{ locale.groups_discussion_replies_edit_thread_raquo }}</a>
								{% endif %}
                <input type="hidden" id="settings_dialog_header" value="{{ locale.groups_discussion_replies_edit_thread_settings }}"/>
				                    <input type="hidden" id="email-verfication-ok" value="1"/> 
    <div class="page-num-list">
		{{ locale.groups_discussion_replies_view_page }}
	{% if currentPage != 1 %}
...
<h1>{{ locale.groups_discussion_replies_oops }}</h1>
...
{% for reply in replyList %}
	...
	<td class="post-list-row-container">
		...
		{% if reply.isOnline() %}
        <img alt="{{ locale.groups_discussion_replies_online_anim }}" src="{{ site.staticContentPath }}/web-gallery/images/myhabbo/habbo_online_anim.gif" />
		{% else %}
		<img alt="{{ locale.groups_discussion_replies_online_anim }}" src="{{ site.staticContentPath }}/web-gallery/images/myhabbo/habbo_offline.gif" />
		{% endif %}
		<div class="post-list-posts post-list-creator-info">{{ locale.groups_discussion_replies_messages }} {{ reply.getForumMessages() }}</div>
		<div class="clearfix">
	...
	<td class="post-list-message" valign="top" colspan="2"> 
		...
		<a href="#" class="quote-post-link verify-email" id="quote-post-{{ reply.getId() }}-message">{{ locale.groups_discussion_replies_quote }}</a>    
		{% endif %}
		...
		{% if (session.loggedIn) and 
				...
				(reply.isDeleted() == false) %}                  
		<a href="#" class="edit-post-link verify-email" id="edit-post-{{ reply.getId() }}-message">{{ locale.groups_discussion_replies_edit }}</a>
		{% endif %}
		
		<span class="post-list-message-header">{% autoescape 'html' %}{% if reply.getId() != firstReply %}{{ locale.groups_discussion_replies_re }} {% endif %}{{ discussionTopic.getTopicTitle() }}{% endautoescape %}</span><br />
       ...
       <div class="post-list-report-element">
        ...
        <div class="post-list-content-element">
			...
			<span class="post-list-message-edited">{{ locale.groups_discussion_replies_last_edited }} {{ reply.getEditedDate('MMM dd, yyyy') }} ({{ reply.getEditedDate('h:mm a') }})</span>
			<br/>
			{% endif %}
			            {%if reply.isDeleted() %}{{ locale.groups_discussion_replies_post_deleted }}{% else %}{{ reply.getFormattedMessage() }}{% endif %}                <input type="hidden" id="{{ reply.getId() }}-message" value="{% autoescape 'html' %}{{ reply.getMessage() }}{% endautoescape %}" />
        </div>
        <div>
        </div>
...
<tr id="new-post-entry-message" style="display:none;">
	...
	<td class="new-post-entry-label"><div class="new-post-entry-label" id="new-post-entry-label">{{ locale.groups_discussion_replies_post }}</div></td>
	<td colspan="2">
        ...
        <a href="#" class="preview-post-link" id="post-form-preview">{{ locale.groups_discussion_replies_preview_raquo }}</a>
        <input type="hidden" id="spam-message" value="{{ locale.groups_discussion_replies_spam_detected }}"/>
...
<div id="linktool-inline">
    <div id="linktool-scope">
        <label for="linktool-query-input">{{ locale.groups_discussion_replies_create_link_to_a }}</label>

        <input type="radio" name="scope" class="linktool-scope" value="1" checked="checked"/>{{ locale.groups_discussion_replies_habbo }}        <input type="radio" name="scope" class="linktool-scope" value="2"/>{{ locale.groups_discussion_replies_room }}        <input type="radio" name="scope" class="linktool-scope" value="3"/>{{ locale.groups_discussion_replies_group_nbsp }}
    </div>
    <div class="linktool-input">
        <input id="linktool-query" type="text" size="30" name="query" value=""/>
        <input id="linktool-find" class="search" type="submit" title="{{ locale.groups_discussion_replies_find }}" value=""/>
    </div>
...
<h3>
<label for="bean_captcha" class="registration-text">{{ locale.groups_discussion_replies_type_in_the_security_code_shown_in_the_image_below }}</label>
</h3>
...
<div class="register-label" id="captcha-reload">
    <p>
        ...
        <a id="captcha-reload-link" href="#">{{ locale.groups_discussion_replies_i_can_t_read_the_code_please_give_me_another_one }}</a>
    </p>
</div>
...
<input type="text" name="captcha" id="captcha-code" value="" class="registration-text required-captcha" />
        ...
        <div class="button-area">
            <a id="post-form-cancel" class="new-button red-button cancel-icon" href="#"><b><span></span>{{ locale.groups_discussion_replies_cancel }}</b><i></i></a>
            <a id="post-form-save" class="new-button green-button save-icon" href="#"><b><span></span>{{ locale.groups_discussion_replies_save }}</b><i></i></a>
        </div>
...
<script type="text/javascript">
L10N.put("myhabbo.discussion.error.topic_name_empty", "{{ locale.groups_discussion_replies_topic_name_cannot_be_empty|escape('js') }}");
L10N.put("register.error.security_code", "{{ locale.groups_discussion_replies_the_security_code_was_invalid_please_try_again|escape('js') }}");
