...
{% endif %}
		<span class="header-left">&nbsp;</span><span class="header-middle">Meine Gruppen ({{ groupsList|length }})</span><span class="header-right">&nbsp;</span></h3>
		</div>	
	</div>
...
Du bist kein Mitglied einer Gruppe.</div>
		{% else %}
...
<li title="{{ group.getName() }}" id="groups-list-{{ sticker.getId() }}-{{ group.getId() }}">
		...
		{% if member.isFavourite(group.id) %}
			<img src="{{ site.staticContentPath }}/web-gallery/images/groups/favourite_group_icon.gif" width="15" height="15" class="groups-list-icon" alt="Favorite" title="Favorit" />
		{% endif %}
		{% if member.getMemberRank().getRankId() == 3 %}
			<img src="{{ site.staticContentPath }}/web-gallery/images/groups/owner_icon.gif" width="15" height="15" class="groups-list-icon" alt="Owner" title="Besitzer" />
		{% endif %}
