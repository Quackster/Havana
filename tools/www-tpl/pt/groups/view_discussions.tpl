...
<div class="box-tabs-container box-tabs-left clearfix">
    ...
    <ul class="box-tabs">
        <li><a href="{{ group.generateClickLink() }}">Página Inicial</a><span class="tab-spacer"></span></li>
        <li class="selected"><a href="{{ group.generateClickLink() }}/discussions">Fórum {% if ((group.getForumType().getId() == 1) or (group.getForumPermission().getId() >= 1)) %}<img src="{{ site.staticContentPath }}/web-gallery/images/grouptabs/privatekey.png" title="Private Forum" alt="Private Forum" />{% endif %}</a><span class="tab-spacer"></span></li>
    </ul>
</div>
...
<div class="topiclist-header clearfix">
		{% if session.loggedIn == false %}
		Por favor entre para portar um novo tópico
		{% elseif canPostForum %}
        <input type="hidden" id="email-verfication-ok" value="1"/>
        <a href="#" id="newtopic-upper" class="new-button verify-email newtopic-icon" style="float:left"><b><span></span>Novo tópico</b><i></i></a>
		{% endif %}
...
<table class="group-topiclist" border="0" cellpadding="0" cellspacing="0" id="group-topiclist-list">
	<tr class="topiclist-columncaption">
		...
		<td class="topiclist-columncaption-lastpost">Última postagem</td>
		<td class="topiclist-columncaption-replies">Respostas</td>
		<td class="topiclist-columncaption-views">Visualizações</td>
	</tr>
{% set num = 0 %}
...
<div class="topiclist-footer clearfix">
        {% if canPostForum %}<a href="#" id="newtopic-lower" class="new-button verify-email newtopic-icon" style="float:left"><b><span></span>Novos Tópicos</b><i></i></a>{% endif %}
    <div class="page-num-list">
    Visualizações da página:
{% if currentPage != 1 %}
<a href="{{ group.generateClickLink() }}/discussions/page/1">&lt;&lt;</a> 
