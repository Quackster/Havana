...
{% if groups|length > 0 %}
		...
		<li class="{{ status }}">
			...
			{% if group.isMember(playerDetails.id) %}
				...
				{% if member.getMemberRank().getRankId() == 3 %}
					<div class="owned-group" title="{{ locale.quickmenu_groups_owner }}"></div>
				{% endif %}
				{% if member.getMemberRank().getRankId() == 2 %}
					<div class="admin-group" title="{{ locale.quickmenu_groups_administrator }}"></div>
				{% endif %}

			{% endif %}
			
			{% if playerDetails.getFavouriteGroupId() == group.id %}
			<div class="favourite-group" title="{{ locale.quickmenu_groups_favorite }}"></div>
			{% endif %}
	...
	{% endfor %}
{% else %}
	<li class="odd">{{ locale.quickmenu_groups_you_don_t_belong_to_any_groups_yet }}</li>
{% endif %}
<p class="create-group"><a href="#" onclick="GroupPurchase.open(); return false;">{{ locale.quickmenu_groups_create_a_group }}</a></p>