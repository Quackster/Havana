...
<ul class="guestbook-entries" id="guestbook-entry-container">
	...
	{% if entries|length == 0 %}
	<div id="guestbook-empty-notes">Dieses Gästebuch hat keine Einträge.</div>
	{% else %}
...
{% if session.loggedIn and sticker.isPostingAllowed(playerDetails.id) %}
	...
	<b><span></span>Neuer Eintrag</b><i></i>
	</a>
	</div>
{% endif %}
{% endif %}
