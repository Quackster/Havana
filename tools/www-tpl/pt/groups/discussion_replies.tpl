{% if hasMessage == false %}
	<div class="postlist-header clearfix">
			...
			{% endif %}
				{% if canReplyForum %}
                    <a href="#" id="create-post-message" class="create-post-link verify-email">Postar Resposta</a>
										{% endif %}
					{% if (session.loggedIn) and ((playerDetails.id == discussionTopic.getCreatorId()) or hasTopicAdmin) %}
                <a href="#" id="edit-topic-settings" class="edit-topic-settings-link">Editar Tópico &raquo;</a>
								{% endif %}
                <input type="hidden" id="settings_dialog_header" value="Edição de Tópico"/>
...
{% for reply in replyList %}
	...
	<td class="post-list-message" valign="top" colspan="2"> 
		...
		{% if (session.loggedIn) and 
				...
				(reply.isDeleted() == false) %}                  
		<a href="#" class="edit-post-link verify-email" id="edit-post-{{ reply.getId() }}-message">Editar</a>
		{% endif %}
