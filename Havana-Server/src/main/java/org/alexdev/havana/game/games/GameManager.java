...
public class GameManager {
     ...
     * Check xp expiry.
     */
public void checkXpExpiry() {
    var currentMonthFormat = DateUtil.getCurrentDate("yyyy-MM");

    if (currentMonthFormat.equalsIgnoreCase(GameConfiguration.getInstance().getString("xp.monthly.expiry"))) {
        return;
    }

    GameConfiguration.getInstance().updateSetting("xp.monthly.expiry", currentMonthFormat);
    GameDao.resetMonthlyXp();
}
