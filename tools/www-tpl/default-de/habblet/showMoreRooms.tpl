...
{% for room in highestRatedRooms %}
    ...
    <span class="clearfix enter-room-link room-occupancy-{{ occupancyLevel }}" title="Zum Raum gehen" roomid="{{ room.getData().getId() }}">
...
{% for room in highestHiddenRatedRooms %}
    ...
    <span class="clearfix enter-room-link room-occupancy-{{ occupancyLevel }}" title="Zum Raum gehen" roomid="{{ room.getData().getId() }}">
	    <span class="room-enter">Enter {{ site.siteName }} Hotel beitreten</span>
	    <span class="room-name">{{ room.getData().getName() }}</span>
	    <span class="room-description">{{ room.getData().getDescription() }}</span>
		<span class="room-owner">Besitzer: <a href="{{ site.sitePath }}/home/{{ room.getData().getOwnerName() }}">{{ room.getData().getOwnerName() }}</a></span>
    </span>
...
{% endfor %}
            ...
            <div class="clearfix">
                <a href="#" class="room-toggle-more-data" id="room-toggle-more-data-h120">Zeige mehr Räume</a>
            </div>
</div>
