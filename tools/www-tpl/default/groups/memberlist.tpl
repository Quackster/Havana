	...
	<li class="{{ status }} online {{ position }}">
    	<div class="item" style="padding-left: 5px; padding-bottom: 4px;">
			...
			{% if member.getMemberRank().getRankId() == 2 %}
				<img src="{{ site.staticContentPath }}/web-gallery/images/groups/administrator_icon.gif" width="15" height="15" alt="{{ locale.groups_memberlist_administrator }}" title="{{ locale.groups_memberlist_administrator }}" />
			{% endif %}
			</div>
...
<div id="member-list-paging" style="margin-top:10px">
	...
	<a href="#" class="avatar-list-paging-link" id="memberlist-search-first" >{{ locale.groups_memberlist_first }}</a> |
	<a href="#" class="avatar-list-paging-link" id="memberlist-search-previous" >&lt;&lt;</a> |
	{% else %}
	{{ locale.groups_memberlist_first_lt_lt }}
	{% endif %}

	{% if (lastPage != -1) or (nextPage != -1) %}
	<a href="#" class="avatar-list-paging-link" id="memberlist-search-next" >&gt;&gt;</a> |
	<a href="#" class="avatar-list-paging-link" id="memberlist-search-last" >{{ locale.groups_memberlist_last }}</a>
	{% else %}
	{{ locale.groups_memberlist_gt_gt_last }}
	{% endif %}
