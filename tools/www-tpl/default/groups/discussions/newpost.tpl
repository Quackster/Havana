...
<tr class="new-topic-entry-label" id="new-topic-entry-label">
	<td class="new-topic-entry-label">{{ locale.groups_discussions_topic }}</td>
	<td colspan="2">
...
<tr id="new-post-entry-message" style="display:none;">
	...
	<td class="new-post-entry-label"><div class="new-post-entry-label" id="new-post-entry-label">{{ locale.groups_discussions_post }}</div></td>
	<td colspan="2">
		<table border="0" cellpadding="0" cellspacing="0" style="margin: 5px; width: 98%;">
		<tr>
		<td>
        ...
        <a href="#" class="preview-post-link" id="topic-form-preview">{{ locale.groups_discussions_preview_raquo }}</a>
...
<div id="linktool-inline">
    <div id="linktool-scope">
        <label for="linktool-query-input">{{ locale.groups_discussions_create_link_to_a }}</label>
        <input type="radio" name="scope" class="linktool-scope" value="1" checked="checked"/>{{ locale.groups_discussions_habbo }}        <input type="radio" name="scope" class="linktool-scope" value="2"/>{{ locale.groups_discussions_room }}        <input type="radio" name="scope" class="linktool-scope" value="3"/>{{ locale.groups_discussions_group_nbsp }}
    </div>
    <div class="linktool-input">
        <input id="linktool-query" type="text" size="30" name="query" value=""/>
        <input id="linktool-find" class="search" type="submit" title="{{ locale.groups_discussions_find }}" value=""/>
    </div>
...
<h3>
<label for="bean_captcha" class="registration-text">{{ locale.groups_discussions_type_in_the_security_code_shown_in_the_image_below }}</label>
</h3>
...
<div class="register-label" id="captcha-reload">
    <p>
        ...
        <a id="captcha-reload-link" href="#">{{ locale.groups_discussions_i_can_t_read_the_code_please_give_me_another_one }}</a>
    </p>
</div>
...
<input type="text" name="captcha" id="captcha-code" value="" class="registration-text required-captcha" />
        ...
        <div class="button-area">
            <a id="topic-form-cancel" class="new-button red-button cancel-icon" href="#"><b><span></span>{{ locale.groups_discussions_cancel }}</b><i></i></a>
            <a id="topic-form-save" class="new-button green-button save-icon" href="#"><b><span></span>{{ locale.groups_discussions_save }}</b><i></i></a>
        </div>
        </td>
        </tr>
        </table>
