...
<ul class="guestbook-entries" id="guestbook-entry-container">
	...
	{% if entries|length == 0 %}
	<div id="guestbook-empty-notes">Ainda não há entradas neste livro de visitas.</div>
	{% else %}
...
{% if session.loggedIn and sticker.isPostingAllowed(playerDetails.id) %}
	...
	<b><span></span>Postar nova mensagem</b><i></i>
	</a>
	</div>
{% endif %}
{% endif %}
