...
<div class="box-tabs-container box-tabs-left clearfix">
	...
	{% else %}
		...
		{% if session.loggedIn == false %}
		<a href="#" id="reporting-button" style="display: none;">Mostrar botões de ajuda</a>
		{% else %}
			{% if (group.isPendingMember(playerDetails.getId()) == false) %}
				{% if (group.isMember(playerDetails.getId()) == false) %}
					...
					<a href="{{ site.sitePath }}/groups/actions/join?groupId=101" id="join-group-button">Entrar</a>
					{% elseif group.getGroupType() == 1 %}
					<a href="{{ site.sitePath }}/groups/actions/join?groupId=101" id="join-group-button">Participar</a>	
					{% endif %}
					<a href="#" id="reporting-button" style="display: none;">Mostrar botões de ajuda</a>
				{% else %}
					{% if group.getOwnerId() != playerDetails.getId() %}
					<a href="{{ site.sitePath }}/groups/actions/leave?groupId=101" id="leave-group-button">Deixar grupo</a>
					{% endif %}
					
					{% if groupMember.isFavourite(group.id) %}
					<a href="#" id="deselect-favorite-button">Remover favorito</a>
					{% else %}
					<a href="#" id="select-favorite-button">Marcar como favorito</a>
					{% endif %}
				{% endif %}
			{% endif %}
		{% endif %}
    ...
    <ul class="box-tabs">
        <li class="selected"><a href="{{ group.generateClickLink() }}">Página Inicial</a><span class="tab-spacer"></span></li>
        <li><a href="{{ group.generateClickLink() }}/discussions">Fórum {% if ((group.getForumType().getId() == 1) or (group.getForumPermission().getId() >= 1)) %}<img src="{{ site.staticContentPath }}/web-gallery/images/grouptabs/privatekey.png" title="Private Forum" alt="Private Forum" />{% endif %}</a><span class="tab-spacer"></span></li>
    </ul>
</div>
