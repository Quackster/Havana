...
{% for room in highestRatedRooms %}
    ...
    <span class="clearfix enter-room-link room-occupancy-{{ occupancyLevel }}" title="{{ locale.habblet_showmorerooms_go_to_room }}" roomid="{{ room.getData().getId() }}">
	    <span class="room-enter">{{ locale.habblet_showmorerooms_enter }} {{ site.siteName }} {{ locale.habblet_showmorerooms_hotel }}</span>
	    <span class="room-name">{{ room.getData().getName() }}</span>
	    <span class="room-description">{{ room.getData().getDescription() }}</span>
		<span class="room-owner">{{ locale.habblet_showmorerooms_owner }} <a href="{{ site.sitePath }}/home/{{ room.getData().getOwnerName() }}">{{ room.getData().getOwnerName() }}</a></span>
    </span>
</li>
{% set num = num + 1 %}
{% endfor %}
...
{% endfor %}
            ...
            <div class="clearfix">
                <a href="#" class="room-toggle-more-data" id="room-toggle-more-data-h120">{{ locale.habblet_showmorerooms_show_more_rooms }}</a>
            </div>
</div>
<script type="text/javascript">
L10N.put("show.more", "{{ locale.habblet_showmorerooms_show_more_rooms|escape('js') }}");
L10N.put("show.less", "{{ locale.habblet_showmorerooms_show_fewer_rooms|escape('js') }}");
