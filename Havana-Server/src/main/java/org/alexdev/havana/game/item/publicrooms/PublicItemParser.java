package org.alexdev.havana.game.item.publicrooms;

import org.alexdev.havana.dao.mysql.PublicRoomsDao;
...
public class PublicItemParser {
    public static List<Item> getPublicItems(int roomId, String modelId) {
        ...
        itemTriggerMap.put("poolEnter", InteractionType.POOL_LADDER);
        itemTriggerMap.put("poolExit", InteractionType.POOL_LADDER);//.POOL_EXIT);
        ...
        for (PublicItemData itemData : publicItemData) {
            ...
            Item item = new Item();
            item.setVirtualId(itemId++);
            item.setRoomId(roomId);
            item.setCustomData(customId);
            item.getDefinition().setSpriteId(-1);
