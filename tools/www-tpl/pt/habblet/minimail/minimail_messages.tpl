	...
	<div class="clearfix labels nostandard">
		<ul class="box-tabs">
			<li{% if minimailLabel == "inbox" %} class="selected"{% endif %}><a href="#" label="inbox">Caixa de entrada</a><span class="tab-spacer"></span></li>
			<li{% if minimailLabel == "sent" %} class="selected"{% endif %}><a href="#" label="sent">Enviados</a><span class="tab-spacer"></span></li>
			<li{% if minimailLabel == "trash" %} class="selected"{% endif %}><a href="#" label="trash">Lixeira</a><span class="tab-spacer"></span></li>
		</ul>

	</div>
	<div id="message-list" class="label-sent">
	<div class="new-buttons clearfix">
		<div class="labels inbox-refresh"><a href="#" class="new-button green-button" label="inbox" style="float: left; margin: 0"><b>Atualizar</b><i></i></a></div>
	</div>
	...
	<div style="clear: both; height: 1px"></div>
		...
		{% if minimailMessages|length > 0 %}
			<div class="trash-controls notice">Messages in this folder that are older than 30 days are deleted automatically. <a href="#" class="empty-trash">Lixeira vazia</a></div>
		{% endif %}
		{% endif %}
		<div class="navigation">
			{% if minimailLabel == "inbox" %}
			<div class="unread-selector"><input type="checkbox" class="unread-only"{% if unreadOnly %} checked="checked"{% endif %}/> apenas não lidos</div>
			{% endif %}
			<!-- <div class="progress"></div> -->
			{% if minimailMessages|length > 0 %}
			<p> 
				{% if showNewest %}<a href="#" class="newest">Mais nova</a>{% endif %}
				{% if showNewer %}<a href="#" class="newer">Nova</a>{% endif %}  {{ startPage }} - {{ endPage }} of {{ totalMessages }}
				{% if showOlder %}<a href="#" class="older">Velha</a>{% endif %}
				{% if showOldest %}<a href="#" class="oldest">Mais velha</a>{% endif %}
			</p>
			{% endif %}
		{% if minimailMessages|length == 0 %}
			{% if minimailLabel == "trash" %}
			<p class="no-messages">Nenhuma mensagem deletada</p>
			{% elseif minimailLabel == "sent" %}
			<p class="no-messages">Nenhuma mensagem enviada</p>
			{% elseif minimailLabel == "conversation" %}
			<p class="no-messages">Nenhuma conversa</p>
			{% else %}
			<p class="no-messages">Nenhuma mensagem</p>
			{% endif %}
		{% endif %}
		</div>
		{% if minimailMessages|length > 0 %}
