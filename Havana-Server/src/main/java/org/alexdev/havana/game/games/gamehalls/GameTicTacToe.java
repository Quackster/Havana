...
import org.alexdev.havana.game.games.gamehalls.utils.GameToken;
    ...
    private void announceWinningSide(Pair<Character, List<int[]>> variables) {
        ...
        if (token != null) {
            ...
            for (Player player : this.getPlayers()) {
                player.send(new CHAT_MESSAGE(ChatMessageType.CHAT, player.getRoomUser().getInstanceId(), winner.getDetails().getName() + " has won the game in " + token.getMoves() + " moves", 0));
            }
