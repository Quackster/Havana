...
public class EntityTask implements Runnable {
    ...
    private void processPet(Pet pet) {
        if (pet.hasActionExpired() && pet.getAction() != PetAction.NONE) {
            ...
            switch (ThreadLocalRandom.current().nextInt(0, 6)) {
                case 1: {
                    ...
                    pet.getRoomUser().removeStatus(StatusType.SLEEP);
        ...
        switch (ThreadLocalRandom.current().nextInt(0, maxRange)) {
            case 2: {
                var talkMessage = PetManager.getInstance().getRandomSpeech(pet);

                if (talkMessage != null) {
                    /*if (pet.getAction() != PetAction.NONE && pet.getAction() != PetAction.SIT && pet.getAction() != PetAction.LAY) {
                        pet.getRoomUser().talk(talkMessage, CHAT_MESSAGE.ChatMessageType.CHAT);
                    } else {*/
                    if (ThreadLocalRandom.current().nextInt(0, 10/* 8 */) == 0) {
                        pet.getRoomUser().talk(talkMessage, CHAT_MESSAGE.ChatMessageType.CHAT);
                    }
                    //}
                }
