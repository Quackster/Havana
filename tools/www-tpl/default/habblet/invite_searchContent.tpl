...
<ul class="habblet-list">
				{% set num = 0 %}
					{% for details in searchResults %}
	            	    ...
	            	    <div class="lastlogin">
	            	    	<b>{{ locale.habblet_invite_searchcontent_last_visit }}</b><br />
	            	    ...
	            	    <div class="tools">
							{% if messenger.hasFriend(details.id) == false %}
	            	    		<a href="#" class="add" avatarid="{{ details.id }}" title="{{ locale.habblet_invite_searchcontent_send_friend_request }}"></a>
							{% endif %}
	            	    </div>
	            	    <div class="clear"></div>
	                </li>
...
<input type="hidden" id="avatar-habblet-list-container-totalPages" value="{{ totalPages }}"/>
				...
				{% else %}
					<div class="box-content">{{ site.siteName }} {{ locale.habblet_invite_searchcontent_not_found_please_make_sure_you_have_typed_his_or_her_name_correctly_and_try_again }} </div>
				{% endif %}