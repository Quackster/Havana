...
public class RoomTile {
    ...
    public static boolean isValidDiagonalTile(Room room, Entity entity, Position position) {
        ...
        if (tile.getHighestItem() != null) {
            ...
            if (!tile.getHighestItem().isWalkable(entity, position)) {
                return false;
            }
        }
    ...
    public boolean hasWalkableFurni(Entity entity) {
        if (this.highestItem != null) {
            return this.highestItem.isWalkable(entity, this.position);
        }
    ...
    public void addItem(Item item)
    {
        ...
        items.put(item.getVirtualId(), item);

        if (item.getTotalHeight() < tileHeight) // TODO: Stack helper?
            return;
    ...
    public void removeItem(Item item)
    {
        ...
        items.remove(item.getVirtualId());

        if (highestItem == null || item.getVirtualId() != highestItem.getVirtualId()) // TODO: Stack helper?
            return;
    ...
    public void resetHighestItem() {
        ...
        // List<Item> tempItems = new ArrayList<>(this.room.getItems());
        // tempItems.sort(Comparator.comparingDouble((Item item) -> item.getPosition().getZ()));

        Item chair = null;//this.items.values().stream().anyMatch(x -> x.getDefinition().hasBehaviour(ItemBehaviour.CAN_SIT_ON_TOP));
        ...
        for (var item : items.values())
        {
            ...
            if (item.getDefinition().hasBehaviour(ItemBehaviour.CAN_SIT_ON_TOP) ||
                    item.getDefinition().hasBehaviour(ItemBehaviour.TELEPORTER) ||
                    item.getDefinition().hasBehaviour(ItemBehaviour.DOOR_TELEPORTER)) {
                chair = item;
            }
        ...
        final Item temp = chair;

        if (chair != null &&
                this.items.values().stream().anyMatch(x -> x.getDatabaseId() != temp.getDatabaseId() && x.getPosition().getZ() > temp.getPosition().getZ())) {
            chairTile = true;
            highestItem = chair;
            tileHeight = chair.getTotalHeight();
        }
    }
    ...
    public double getWalkingHeight() {
        ...
        var highestItem = this.highestItem;

        if (highestItem != null) {
            if (highestItem.hasBehaviour(ItemBehaviour.CAN_SIT_ON_TOP) || highestItem.hasBehaviour(ItemBehaviour.CAN_LAY_ON_TOP)) {
                height -= highestItem.getDefinition().getPositiveTopHeight();
            }
        }
    ...
    public ArrayList<Item> getItemsAbove(Item item) {
        var items = getItems();
        items.removeIf(x -> x.getDatabaseId() == item.getDatabaseId() || x.getPosition().getZ() < item.getPosition().getZ());
        return items;
    }

    public boolean isChairTile() {
        return chairTile;
    }

    public void setChairTile(boolean chairTile) {
