{% include "housekeeping/base/header.tpl" %}
  <body>
	...
	{% include "housekeeping/base/navigation.tpl" %}
		...
		<p>Editar una encuesta de Infobus</p>
		<form class="table-responsive col-md-4" method="post">
			...
			<p><button type="button" class="btn btn-success" onclick="cloneAnswer()">Agregar respuesta</button></p>
			<p><button type="button" class="btn btn-primary" onclick="removeLatestAnswer()">Eliminar la respuesta</button></p>
			
			<div class="form-group"> 
				<input type="hidden" id="text" name="creator" value="{{ playerDetails.id }}">
				<button type="submit" class="btn btn-info">Guardar encuesta</button>
			</div>
		</form>
      </div>
    </div>
