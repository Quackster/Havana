...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="{{ locale.global_html_lang }}" lang="{{ locale.global_html_lang }}">
   <head>
      ...
      <title>{{ site.siteName }} {{ locale.index_chromide_home }} </title>
      ...
      <meta name="description" content="{{ locale.index_chromide_habbo_is_a_virtual_world_where_you_can_meet_and_make_friends }}" />
      <meta name="keywords" content="{{ locale.index_chromide_habbo_virtual_world_play_games_enter_competitions_make_friends }}" />
      <!--[if IE 8]>
      ...
      <![endif]-->
      <meta name="build" content="{{ locale.index_chromide_three_two_build_eight_two_zero_six_zero_four_two_zero_zero_nine_one_one_five_one_uk }}" />
   </head>
   <body id="frontpage">
      <div id="fp-container">
         <div id="header" class="clearfix">
            ...
            {{ locale.index_chromide_new_here }}
            <a href="{{ site.sitePath }}/register">{{ locale.index_chromide_register_for_free }}</a>
            </span>
         </div>
         <div id="content">
   ...
   <li>{{ alert.message }}</li>
				...
				{% endif %}
                     <div class="cbb loginbox clearfix">
                        <h2 class="title">{{ locale.index_chromide_sign_in }}</h2>
                        <div class="box-content clearfix" id="login-habblet">
                           <form action="{{ site.sitePath }}/account/submit" method="post" class="login-habblet">
                              <ul>
                                 <li>
                                    <label for="login-username" class="login-text">{{ locale.index_chromide_username }}</label>
                                    <input tabindex="1" type="text" class="login-field" name="username" id="login-username" value="{{ username }}" maxlength="32"/>
                                 </li>
                                 <li>
                                    <label for="login-password" class="login-text">{{ locale.index_chromide_password }}</label>
                                    <input tabindex="2" type="password" class="login-field" name="password" id="login-password" maxlength="32"/>
                                    <input type="submit" value="{{ locale.index_chromide_sign_in }}" class="submit" id="login-submit-button"/>
                                    <a href="#" id="login-submit-new-button" class="new-button" style="margin-left: 0;display:none"><b style="padding-left: 10px; padding-right: 7px; width: 55px">{{ locale.index_chromide_sign_in }}</b><i></i></a>
                                 </li>
                                 <li id="remember-me" class="no-label">
									...
									{% endif %}
                                    <label for="login-remember-me">{{ locale.index_chromide_remember_me }}</label>
                                 </li>
                                 <li id="register-link" class="no-label">
                                    <a href="{{ site.sitePath }}/register" class="login-register-link"><span>{{ locale.index_chromide_register_for_free_text }}</span></a>
                                 </li>
                                 <li class="no-label">
                                    <a href="{{ site.sitePath }}/account/password/forgot" id="forgot-password"><span>{{ locale.index_chromide_i_forgot_my_password_username }}</span></a>
                                 </li>
                              </ul>
                              <div id="remember-me-notification" class="bottom-bubble" style="display:none;">
                                 ...
                                 <div class="bottom-bubble-c">
                                    {{ locale.index_chromide_by_selecting_remember_me_you_will_stay_signed_in_on_this_computer_until_you_click_sign_out_if_this_is_a_public_computer_please_do_not_use_this_feature }}
                                 </div>
                  ...
                  <script type="text/javascript">
                     L10N.put("authentication.form.name", "{{ locale.index_chromide_username|escape('js') }}");
                     L10N.put("authentication.form.password", "{{ locale.index_chromide_password|escape('js') }}");
               ...
               <div class="habblet-container ">
                  ...
                  <script type="text/javascript">
                     ...
                     var i = 0;
					sb.add("fp-bubble-"+i++, "frontpage-image", 489, 143, "{{ locale.index_chromide_create_your_own_habbo|escape('js') }}");
					sb.add("fp-bubble-"+i++, "frontpage-image", 478, 254, "{{ locale.index_chromide_and_make_new_friends|escape('js') }}");
					sb.add("fp-bubble-"+i++, "frontpage-image", 93, 249, "{{ locale.index_chromide_join_us_in_habbo_world_s_biggest_virtual_world|escape('js') }}");
            ...
            <div id="column-footer">
               <div class="habblet-container ">
                  <div class="cbb" id="hotel-stats">
                     <ul class="stats">
                        <li class="stats-online"><span class="stats-fig">{{ site.usersOnline }}</span> {{ site.siteName }}{{ locale.index_chromide_s_online_now }}</li>
                        <li class="stats-visited"><span class="stats-fig">{{ site.visits }}</span> {{ locale.index_chromide_visits_in_the_last_three_zero_days }}</li>
                     </ul>
                  </div>
               </div>
               <script type="text/javascript">if (!$(document.body).hasClassName('process-template')) { Rounder.init(); }</script>
               <div class="habblet-container ">
                  <div class="cbb habblet box-content" id="tag-cloud-slim">
                     <span class="tags-habbos-like">{{ site.siteName }}{{ locale.index_chromide_s_like }}</span>
                     <ul class="tag-list">
						{% autoescape 'html' %}
						...
						{% endfor %}
						{% else %}
						<li><span class="tags-habbos-like">{{ locale.index_chromide_no_tags_to_display }}</span></li>
						{% endif %}
						{% endautoescape %}
                     </ul>
                  </div>
      ...
      <script type="text/javascript">
         if (window.location.href.indexOf("/register/welcome") != -1) {
         	...
         	document.write('<' + '{{ locale.index_chromide_script_language_javascript_type_text_javascript_src_https_www_googleadservices_com_pagead_conversion_js|escape('js') }}' +        '>' + '</' + 'script' + '>');
         	...
         	document.write('<' + '{{ locale.index_chromide_script_language_javascript_type_text_javascript_src_https_zero_r_msn_com_scripts_microsoft_adcenterconversion_js|escape('js') }}' +        '>' + '</' + 'script' + '>');	
      ...
      <noscript>
         <a href="http://www.quantcast.com/p-b5UDx6EsiRfMI" target="_blank"><img src="http://pixel.quantserve.com/pixel/p-b5UDx6EsiRfMI.gif" style="display: none" border="0" height="1" width="1" alt="{{ locale.index_chromide_quantcast }}"/></a>
      </noscript>
      <!-- End Quantcast tag -->
   </body>
</html>