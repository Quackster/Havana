...
{% endif %}
		<span class="header-left">&nbsp;</span><span class="header-middle">{{ locale.homes_widget_my_rooms }}</span><span class="header-right">&nbsp;</span></h3>
		</div>	
	</div>
	<div class="widget-body">
		<div class="widget-content">
		{% if rooms|length == 0 %}
		{{ locale.homes_widget_you_do_not_have_any_rooms_yet }}
		{% else %}
    <div id="room_wrapper">
        	...
        	<div class="room_info">
        		...
        		<div>{% autoescape 'html' %}{{ room.getData().getDescription() }}{% endautoescape %}        		</div>
					<a href="{{ site.sitePath }}/client?forwardId=2&amp;roomId={{ room.getId() }}"
					   ...
					   onclick="HabboClient.roomForward(this, '{{ room.getId() }}', 'private', false); return false;">					 {{ locale.homes_widget_enter_room }}					 </a>        	</div>
		<br class="clear" />
