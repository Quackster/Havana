...
<div class="box-tabs-container box-tabs-left clearfix">
	...
	{% else %}
		...
		{% if session.loggedIn == false %}
		<a href="#" id="reporting-button" style="display: none;">Melden</a>
		{% else %}
			{% if (group.isPendingMember(playerDetails.getId()) == false) %}
				{% if (group.isMember(playerDetails.getId()) == false) %}
					...
					<a href="{{ site.sitePath }}/groups/actions/join?groupId=101" id="join-group-button">Mitgliedschaft anfragen</a>	
					{% endif %}
					<a href="#" id="reporting-button" style="display: none;">Melden</a>
				{% else %}
    ...
    <ul class="box-tabs">
        <li><a href="{{ group.generateClickLink() }}">Startseite</a><span class="tab-spacer"></span></li>
        <li class="selected"><a href="{{ group.generateClickLink() }}/discussions">Diskussionsforum {% if ((group.getForumType().getId() == 1) or (group.getForumPermission().getId() >= 1)) %}<img src="{{ site.staticContentPath }}/web-gallery/images/grouptabs/privatekey.png" title="Private Forum" alt="Private Forum" />{% endif %}</a><span class="tab-spacer"></span></li>
    </ul>
</div>
...
<p>
        Forenzugriff verweigert. Bitte prüfe, ob du angemeldet bist und über die entsprechenden Rechte zum Anzeigen der Foren verfügst. Wenn du angemeldet bist und die Foren trotzdem nicht angezeigt werden, handelt es sich möglicherweise um eine private Gruppe. In diesem Fall musst du der Gruppe beitreten um die Foren anzeigen zu können. 
<br/>
</p>
...
<div class="topiclist-header clearfix">
		{% if session.loggedIn == false %}
		Bitte melden sich an, um neue Themen zu veröffentlichen.
		{% elseif canPostForum %}
        <input type="hidden" id="email-verfication-ok" value="1"/>
        <a href="#" id="newtopic-upper" class="new-button verify-email newtopic-icon" style="float:left"><b><span></span>Neuer Thread</b><i></i></a>
		{% endif %}
...
<table class="group-topiclist" border="0" cellpadding="0" cellspacing="0" id="group-topiclist-list">
	<tr class="topiclist-columncaption">
		<td class="topiclist-columncaption-topic">Ersteller:</td>
		<td class="topiclist-columncaption-lastpost">Letzter Post:</td>
		<td class="topiclist-columncaption-replies">Antworten:</td>
		<td class="topiclist-columncaption-views">Ansichten:</td>
	</tr>
{% set num = 0 %}
{% for topic in discussionTopics %}
	...
	{% endif %}
		<td class="topiclist-rowtopic" valign="top">
			...
			<span class="topiclist-row-topicnew">NEU <img src="{{ site.staticContentPath }}/web-gallery/images/discussions/New_arrow.gif" alt="NEW"/></span>{% endif %}			</div>
							</div>
		...
		<td class="topiclist-lastpost" valign="top">
			...
			<span class="topiclist-row-writtenby">Von:</span> <a class="topiclist-row-openername" href="{{ site.sitePath }}/home/{{ topic.getLastReplyName() }}">{{ topic.getLastReplyName() }}</a>
...
<div class="topiclist-footer clearfix">
        {% if canPostForum %}<a href="#" id="newtopic-lower" class="new-button verify-email newtopic-icon" style="float:left"><b><span></span>Neuer Thread</b><i></i></a>{% endif %}
    <div class="page-num-list">
    Seite:
{% if currentPage != 1 %}
<a href="{{ group.generateClickLink() }}/discussions/page/1">&lt;&lt;</a> 
...
<script type="text/javascript">
L10N.put("myhabbo.discussion.error.topic_name_empty", "Der Themenname darf nicht leer sein");
L10N.put("register.error.security_code", "Der Sicherheitscode war ungültig. Bitte versuche es erneut..");
...
<div class="cbb topdialog" id="guestbook-form-dialog">
	<h2 class="title dialog-handle">Eintrag bearbeiten</h2>
...
<div id="linktool">
    <div id="linktool-scope">
        <label for="linktool-query-input">Link erstellen:</label>
    ...
    <a href="#" class="new-button" id="linktool-find"><b>Suchen</b><i></i></a>
...
<ul class="box-tabs">
	...
	<li id="group-memberlist-link-pending"><a href="#">Mitgliedsanfragen</a><span class="tab-spacer"></span></li>
</ul>
</div>
...
<div id="group-memberlist-members-buttons" class="clearfix">
	...
	<a href="#" class="new-button group-memberlist-button-disabled" id="group-memberlist-button-revoke-rights"><b>Rechte entfernen</b><i></i></a>{% endif %}
...
<div id="group-memberlist-pending-buttons" class="clearfix">
	<a href="#" class="new-button group-memberlist-button-disabled" id="group-memberlist-button-accept"><b>Annehmen</b><i></i></a>
