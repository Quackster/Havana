package org.alexdev.havana.game.pathfinder;
...
public class Pathfinder {
    ...
    public static boolean isValidStep(Room room, Entity entity, Position current, Position tmp, boolean isFinalMove) {
        ...
        if (!current.equals(room.getModel().getDoorLocation()) &&
                !tmp.equals(room.getModel().getDoorLocation())) {
            if (toItem != null) {
                if (isFinalMove) {
                    // Allow walking if item is walkable or trapped inside
                    return toItem.isWalkable(entity, tmp) || AffectedTile.getAffectedTiles(toItem).stream().anyMatch(x -> room.getMapping().getTile(x).containsEntity(entity));
                } else {
                    return toItem.hasBehaviour(ItemBehaviour.CAN_STAND_ON_TOP) || toItem.hasBehaviour(ItemBehaviour.DOOR_TELEPORTER) || toItem.isGateOpen();
                }
            }
        }
