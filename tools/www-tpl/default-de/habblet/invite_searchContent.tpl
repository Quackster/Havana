					...
					{% for details in searchResults %}
	            	    ...
	            	    <div class="tools">
							{% if messenger.hasFriend(details.id) == false %}
	            	    		<a href="#" class="add" avatarid="{{ details.id }}" title="Anfrage senden"></a>
							{% endif %}
	            	    </div>
	            	    <div class="clear"></div>
	                </li>
...
<input type="hidden" id="avatar-habblet-list-container-totalPages" value="{{ totalPages }}"/>
				...
				{% else %}
					<div class="box-content">{{ site.siteName }} nicht gefunden. Bitte überprüfe, ob du den Namen richtig eingegeben hast und versuchen es erneut. </div>
				{% endif %}