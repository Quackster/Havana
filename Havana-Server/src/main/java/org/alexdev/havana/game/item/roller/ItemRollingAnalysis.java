...
public class ItemRollingAnalysis implements RollingAnalysis<Item> {
    ...
    public Position canRoll(Item item, Item roller, Room room) {
        if (roller == null || roller.getRoom() == null || roller.getTile() == null) {
            return null;
        }

        if (item.getVirtualId() == roller.getVirtualId()) {
            return null;
        }
        ...
        if (frontTile.getHighestItem() != null) {
            ...
            if (frontRoller != null) {
                ...
                for (Item frontItem : frontTile.getItems()) {
                    ...
                    if (frontItem.getDatabaseId() == item.getDatabaseId()) {
                        continue;
                    }
                    ...
                    if (frontItem.hasBehaviour(ItemBehaviour.ROLLER)) {
                        ...
                        if (frontPosition.equals(item.getPosition())) {
                            if (frontTile.getItemsAbove(frontRoller).size() > 0 || frontTile.getEntireEntities().size() > 0) {
                                return null;
                ...
                if (!highestNextItem.hasBehaviour(ItemBehaviour.ROLLER) && highestNextItem.getDatabaseId() != item.getDatabaseId()) {
                    if (highestNextItem.hasBehaviour(ItemBehaviour.CAN_STACK_ON_TOP)
                            && item.getTile().getEntireEntities().isEmpty()) {
    ...
    public void doRoll(Item item, Item roller, Room room, Position fromPosition, Position nextPosition) {
        ...
        //room.send(new SLIDE_OBJECT(item, nextPosition, roller.getVirtualId(), nextPosition.getZ()));
