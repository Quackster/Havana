...
{% endif %}
		<span class="header-left">&nbsp;</span><span class="header-middle">{{ locale.homes_widget_my_profile }}</span><span class="header-right">&nbsp;</span></h3>
		</div>	
	</div>
	...
	<div class="profile-info">		
		...
		{% if user.isOnline() and user.isProfileVisible() %}
					<img alt="{{ locale.homes_widget_online }}" src="{{ site.staticContentPath }}/web-gallery/images/myhabbo/profile/habbo_online_anim.gif" />
		{% else %}
					<img alt="{{ locale.homes_widget_online }}" src="{{ site.staticContentPath }}/web-gallery/images/myhabbo/profile/habbo_offline.gif" />
		{% endif %}
		<div class="birthday text">
			{{ locale.homes_widget_created_on }}
		</div>
	...
	{% endautoescape %}
				...
				<div class="profile-friend-request clearfix">
						<a class="new-button" id="add-friend" style="float: left"><b>{{ locale.homes_widget_add_as_friend }}</b><i></i></a>
					</div>
				{% endif %}
 ...
 <div style="display: block;">
  <div class="content-red">
   <div class="content-red-body">
    <span id="tag-limit-message"><img src="{{ site.staticContentPath }}/web-gallery/images/register/icon_error.gif"/> {{ locale.homes_widget_the_limit_is_eight_tags }}</span>
    ...
    <script type="text/javascript">
		document.observe("dom:loaded", function() {
			new ProfileWidget('{{ user.id }}', '{{ playerDetails.id }}', {
				headerText: "{{ locale.homes_widget_are_you_sure|escape('js') }}",
				messageText: "{{ locale.homes_widget_are_you_sure_you_want_to_ask_strong|escape('js') }}{{ user.getName() }}{{ locale.homes_widget_strong_to_be_your_friend_think_twice_before_you_hit_ok|escape('js') }}",
				loginText: "{{ locale.homes_widget_you_must_sign_in_before_sending_a_friend_request|escape('js') }}",
				buttonText: "{{ locale.homes_widget_ok|escape('js') }}",
				cancelButtonText: "{{ locale.homes_widget_cancel|escape('js') }}"
			});
		});
	</script>
	{% endif  %}
